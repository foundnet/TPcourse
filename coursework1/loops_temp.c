...
int main(int argc, char *argv[]) { 
  ...
  char fname[100];
  char sline[100];
  sprintf(fname,"logs/Loop1_%s_%d.csv",stype,size_loop1);
  FILE *pf = fopen(fname , "w");
  if (pf != NULL) {
    for (int m = 0;m < N;m++) {
      sprintf(sline,"DISTLP1,%s,%d,%d,%d\n",stype,size_loop1,dist_loop1[m],m);
      fwrite(sline,strlen(sline),1,pf);
    }
    fclose(pf);
  }
  else printf ("%s\n",fname);
  sprintf(fname,"logs/Loop2_%s_%d.csv",stype,size_loop2);
  pf = fopen(fname , "w");
  if (pf != NULL) {
    for (int m = 0;m < N;m++) {
      sprintf(sline,"DISTLP2,%s,%d,%d,%d\n",stype,size_loop2,dist_loop2[m],m);
      fwrite(sline,strlen(sline),1,pf);
    }
    fclose(pf);
  }
